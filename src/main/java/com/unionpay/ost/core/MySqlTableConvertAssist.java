...
public class MySqlTableConvertAssist {
     ...
     */
    public static List<FieldMeta> obtainFieldMetasForMySQL(Connection connection, String tableName, String dbType, List<EntityMeta> entityMetaList) {
        ...
        try {
            ...
            if (pkCount > 1) {
                ...
                priKeyFieldMetaList = obtainPriKeyFieldMetaForManyKey(rs);
                //设置新增实体实体名
                entityMeta.setEntityName(StringUtils.capInMark(tableName, DataBaseRelateConstant.TABLESPILTMARK, true) + DataBaseRelateConstant.PRIKEYTABLESUFFIX);
                entityMeta.setFieldMetaList(priKeyFieldMetaList);
                System.out.println("复合主键,<成功生成"+entityMeta.getEntityName()+"实体类>");
                entityMetaList.add(entityMeta);
            }
            fieldMetaList = obtainFieldMetasFromMySqlResultSet(connection, pkCount, tableName);
     ...
     * @return 属性元数据集合
     */
    private static List<FieldMeta> obtainFieldMetasFromMySqlResultSet(Connection connection, int pkCount, String tableName) {
        List<FieldMeta> fieldMetaList = new ArrayList<FieldMeta>();
        //对于多主键情况,是否有新属性生成
...
//                        }
     ...
     * @return 新实体的属性集合
     */
    private static List<FieldMeta> obtainPriKeyFieldMetaForManyKey(ResultSet rs) {
        List<FieldMeta> fieldMetaList = new ArrayList<FieldMeta>();
        if (rs != null) {
            try {
                while (rs.next()) {
